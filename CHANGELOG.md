...
- `pps remote`가 PPS 로그인과 해당 문제의 관리 권한을 확인한 뒤에만 로컬 Git 저장소를 연결합니다. 공개 문제 목록에 보인다는 이유만으로 연결을 허용하던 대체 경로를 제거했습니다.

### 개선

- 연결 권한이 없을 때 문제 ID가 포함된 명확한 권한 오류를 반환하여 PPS Code 같은 클라이언트가 로그인 오류와 구분해 안내할 수 있습니다.
