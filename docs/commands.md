...
PPS CLI에서 대화형 입력을 사용할 수 있는 명령은 `pps auth`와 `pps create`뿐입니다. 나머지 명령은 질문을 띄우지 않으며 위치 인자, 명시적인 옵션, 문서화된 기본값만 사용합니다. 따라서 터미널과 CI에서 같은 명령을 그대로 재현할 수 있습니다.
...
`auth`와 `create`를 제외한 명령은 입력 메뉴를 띄우지 않습니다. 필요한 ID와 필터를 옵션으로 명시하고, 구조화된 결과에는 `--json`을 사용하세요.
