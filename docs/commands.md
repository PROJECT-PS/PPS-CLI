...
같은 URL이 이미 설정되어 있으면 오류 없이 메타데이터만 다시 기록합니다. 다른 URL의 remote는 `--force` 없이는 절대 바꾸지 않습니다. 명령은 pull, fetch, push와 작업 파일 변경을 수행하지 않습니다. 연결에는 PPS 로그인이 필요하며, 로그인한 계정이 해당 문제의 제작자 또는 관리 권한을 가진 계정이어야 합니다. 공개 문제를 조회하거나 clone할 수 있더라도 관리 권한이 없으면 연결할 수 없습니다. 자동화 결과는 `pps --json remote 123`으로 받을 수 있습니다.
